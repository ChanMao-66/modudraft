...
- `modudraft-core.js` 內新增 `KITCHEN_RULES`，集中管理廚具施工尺寸與一字型配置規則。下櫃完成高、踢腳、桶身、檯面厚度、下櫃深度、檯面深度、吊櫃高度與深度不再由新手流程自行寫死。
- 廚具入口分成「新手快速建立」、「專業空白工程」、「從範本開始」與「匯入既有專案」。
- 新手模式採七步驟：型態、尺寸、設備、生成、檢查、風格、輸出；一字型完整支援，L 型提供穩定簡化版，U 型與中島明確標示即將支援。
- `kitchen-workflow.js / kitchen-workflow.css` 提供標準櫃資料庫、規則式基礎配置、櫃體清單、CSV／JSON、AI 提案提示詞與手機卡片流程；一字型配置已使用 `KITCHEN_RULES` 的固定水槽、爐台、中間櫃與補板規則。
...
- 最小必要設備優先保留 600 mm 水槽下櫃與 600 mm 爐台下櫃；系統不得為了塞入其他櫃體而自動縮小固定設備。
- 完整基本配置以「左補板 + 600 水槽櫃 + 500 中間櫃 + 600 爐台櫃 + 右補板」為基礎；預設補板寬度來自 `KITCHEN_RULES.straight.defaultEdgeFillerWidth`，後續可開放成專案設定。
- 空間不足時，先取消預設補板，再移除中間非必要櫃；仍不足時停止產生可靠配置並顯示缺少尺寸。
- 空間足夠時，水槽可優先升級至 800 mm、爐台可優先升級至 700 mm；剩餘中間段依 `preferredStorageMaxWidth` 拆成可施工的抽屜／收納櫃。
- 水槽與爐台距離目前使用 `KITCHEN_RULES.sinkStove.suggestedMinimumDistance` 作為提醒值，不作為硬性阻止規則。
