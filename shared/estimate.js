  ...
  function mount(options = {}) {
    ...
    overlay.innerHTML = `
      <section class="md-estimate-panel" role="dialog" aria-modal="true" aria-label="MODUDRAFT 廚具與系統櫃粗估價">
        <header class="md-estimate-header">
          ...
          <button type="button" class="md-estimate-close" aria-label="關閉估價" data-help-id="estimate">×</button>
        </header>
        <div class="md-estimate-stale" hidden><b>目前配置已變更</b><span>是否重新依配置產生估價？手動修改的明細可能會被更新。</span><button type="button" data-estimate-action="regenerate" data-help-id="estimate-regenerate">重新產生</button></div>
        <nav class="md-estimate-tabs" aria-label="估價功能"><button class="active" data-estimate-tab="overview" data-help-id="estimate-overview">總覽</button><button data-estimate-tab="details" data-help-id="estimate-details">明細</button><button data-estimate-tab="rates" data-help-id="estimate-rates">單價設定</button><button data-estimate-tab="manual" data-help-id="estimate-manual">手動新增</button></nav>
        <main class="md-estimate-content"></main>
        <footer class="md-estimate-footer"><div class="md-estimate-footer-totals"><span>合計 <b data-total="subtotal">NT$0</b></span><span>稅額 <b data-total="tax">NT$0</b></span><span class="grand">總計 <b data-total="grand">NT$0</b></span></div><div class="md-estimate-footer-actions"><button data-estimate-action="copy" data-help-id="estimate-copy">複製明細</button><button data-estimate-action="csv" data-help-id="estimate-csv">CSV／Excel</button><button data-estimate-action="json" data-help-id="estimate-json">匯出 JSON</button></div></footer>
      </section>`;
    ...
    function overviewHtml() {
      ...
      return `<section class="md-estimate-overview">
        ...
        <div class="md-estimate-settings-grid"><label>稅額模式<select data-document-field="taxMode" data-help-id="estimate-tax-mode"><option value="excluded" ${document.taxMode === "excluded" ? "selected" : ""}>未稅另加</option><option value="included" ${document.taxMode === "included" ? "selected" : ""}>金額已含稅</option><option value="none" ${document.taxMode === "none" ? "selected" : ""}>不計稅</option></select></label><label>稅率<span><input data-document-field="taxRate" data-help-id="estimate-tax-mode" type="number" min="0" max="100" step="0.1" value="${roundQuantity(document.taxRate * 100, 2)}"><b>%</b></span></label>${adapter.type === "kitchen" ? `<label>檯面材質<select data-setting-field="countertopMaterial" data-help-id="estimate-plan">${Object.entries(COUNTERTOP_OPTIONS).map(([id, option]) => `<option value="${id}" ${document.settings.countertopMaterial === id ? "selected" : ""}>${option.name}</option>`).join("")}</select></label><label>三機方案<select data-setting-field="appliancePlan" data-help-id="estimate-plan"><option value="none" ${document.settings.appliancePlan === "none" ? "selected" : ""}>不含三機</option><option value="entry" ${document.settings.appliancePlan === "entry" ? "selected" : ""}>入門三機</option><option value="standard" ${document.settings.appliancePlan === "standard" ? "selected" : ""}>標準三機</option><option value="premium" ${document.settings.appliancePlan === "premium" ? "selected" : ""}>高階三機</option><option value="custom" ${document.settings.appliancePlan === "custom" ? "selected" : ""}>自訂三機／待報價</option></select></label><label>水槽龍頭<select data-setting-field="sinkFaucetPlan" data-help-id="estimate-plan"><option value="none" ${document.settings.sinkFaucetPlan === "none" ? "selected" : ""}>不含／客供</option><option value="separate" ${document.settings.sinkFaucetPlan === "separate" ? "selected" : ""}>基本水槽＋龍頭</option><option value="set" ${document.settings.sinkFaucetPlan === "set" ? "selected" : ""}>標準水槽龍頭組</option></select></label>` : ""}<label>運送方案<select data-setting-field="deliveryPlan" data-help-id="estimate-plan"><option value="none" ${document.settings.deliveryPlan === "none" ? "selected" : ""}>暫不計入</option><option value="basic" ${document.settings.deliveryPlan === "basic" ? "selected" : ""}>基本運送</option><option value="premium" ${document.settings.deliveryPlan === "premium" ? "selected" : ""}>高階車資／運送</option></select></label></div>
        <button type="button" class="md-estimate-regenerate" data-estimate-action="regenerate" data-help-id="estimate-regenerate">依目前配置重新產生估價</button>
    ...
    function itemCard(item) {
      ...
      return `<article class="md-estimate-item ${item.includeInTotal ? "" : "excluded"}" data-item-id="${escapeHtml(item.id)}"><div class="md-estimate-item-head"><span>${item.itemNo}</span><div><input data-item-field="name" data-help-id="estimate-item" value="${escapeHtml(item.name)}" aria-label="品名"><input data-item-field="spec" data-help-id="estimate-item" value="${escapeHtml(item.spec)}" aria-label="規格"></div><b>${item.pricingStatus === "priced" ? money(item.subtotal) : statusLabel(item.pricingStatus)}</b></div><div class="md-estimate-item-grid"><label>單位<select data-item-field="unit" data-help-id="estimate-item">${UNITS.map((unit) => `<option ${item.unit === unit ? "selected" : ""}>${unit}</option>`).join("")}</select></label><label>數量<input data-item-field="quantity" data-help-id="estimate-item" type="number" inputmode="decimal" min="0" step="0.1" value="${item.quantity}"></label><label>單價<input data-item-field="unitPrice" data-help-id="estimate-item" type="number" inputmode="numeric" min="0" step="1" value="${item.unitPrice}"></label><label>狀態<select data-item-field="pricingStatus" data-help-id="estimate-item">${statusOptions}</select></label></div><label class="md-estimate-note">備註<input data-item-field="note" data-help-id="estimate-item" value="${escapeHtml(item.note)}"></label><div class="md-estimate-item-foot"><label><input data-item-field="includeInTotal" data-help-id="estimate-item" type="checkbox" ${item.includeInTotal ? "checked" : ""}>計入總價</label>${item.sourceType === "manual" ? `<button type="button" data-estimate-action="delete-item" data-help-id="estimate-item">刪除此項</button>` : `<span>來源：${escapeHtml(item.sourceType)}</span>`}</div></article>`;
    }
    ...
    function ratesHtml() {
      return `<section class="md-estimate-rates"><div class="md-estimate-rate-intro"><h3>專案單價設定</h3><p>修改後按「套用單價並重新產生」，不會影響其他專案。</p></div>${Object.entries(RATE_LABELS).map(([group, labels]) => `<details ${["kitchen", "countertop", "appliance", "systemCabinet"].includes(group) ? "open" : ""}><summary>${escapeHtml({ kitchen: "廚具櫃身", countertop: "檯面", processing: "加工與安裝", sinkFaucet: "水槽與龍頭", handle: "把手", hardware: "五金", appliance: "三機設備", other: "其他費用", systemCabinet: "系統櫃", systemAccessory: "系統櫃配件" }[group] || group)}</summary><div class="md-estimate-rate-grid">${Object.entries(labels).map(([key, label]) => `<label>${escapeHtml(label)}<span><input type="number" min="0" step="1" data-help-id="estimate-rates" data-rate-group="${group}" data-rate-key="${key}" value="${safeNumber(currentDocument.rates?.[group]?.[key], 0)}"><b>NT$</b></span></label>`).join("")}</div></details>`).join("")}<div class="md-estimate-rate-actions"><button type="button" data-estimate-action="reset-rates" data-help-id="estimate-rates">恢復系統預設</button><button type="button" class="primary" data-estimate-action="apply-rates" data-help-id="estimate-rates">套用單價並重新產生</button></div></section>`;
    }

    function manualHtml() {
      return `<section class="md-estimate-manual"><div><span>手動明細</span><h3>新增估價項目</h3><p>用於玻璃加工、特殊五金或現場臨時費用。所有金額仍會進入同一套明細加總。</p></div><form id="mdEstimateManualForm" data-help-id="estimate-manual"><label>分類<select name="sectionId" data-help-id="estimate-manual">${SECTION_DEFINITIONS.map((section) => `<option value="${section.id}" ${section.id === "manual" ? "selected" : ""}>${section.title}</option>`).join("")}</select></label><label>名稱<input name="name" data-help-id="estimate-manual" required placeholder="例如：玻璃條工資"></label><label>規格<input name="spec" data-help-id="estimate-manual" placeholder="選填"></label><label>單位<select name="unit" data-help-id="estimate-manual">${UNITS.map((unit) => `<option>${unit}</option>`).join("")}</select></label><label>數量<input name="quantity" data-help-id="estimate-manual" type="number" inputmode="decimal" min="0" step="0.1" value="1" required></label><label>單價<input name="unitPrice" data-help-id="estimate-manual" type="number" inputmode="numeric" min="0" step="1" value="0" required></label><label>計價狀態<select name="pricingStatus" data-help-id="estimate-manual">${Object.entries(PRICING_STATUSES).map(([value, label]) => `<option value="${value}">${label}</option>`).join("")}</select></label><label class="wide">備註<input name="note" data-help-id="estimate-manual" placeholder="選填"></label><label class="check"><input name="includeInTotal" data-help-id="estimate-manual" type="checkbox" checked>計入總價</label><button type="submit" class="primary" data-help-id="estimate-manual">新增到估價明細</button></form></section>`;
    }
