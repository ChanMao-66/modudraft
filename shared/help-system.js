(function (global) {
  ...
  defineHelp("estimate", "模擬估價", "估價", "依目前廚具或系統櫃配置產生逐項估價明細、稅額與總計。", "配置完成後，想先了解櫃身、檯面、三機、五金與人工的粗估金額時使用。", ["點估價開啟 MODUDRAFT Estimate。", "確認檯面、三機與運送方案。", "到明細調整數量、單價與計價狀態。", "需要時新增特殊工項。", "複製明細或匯出 CSV／JSON。"], ["只有狀態為計價且勾選計入總價的項目會加總。", "配置變更後系統只會提示重新產生，不會偷偷覆蓋手動修改。", "此為線上模擬估價，仍需廠商正式報價。"], "把油煙機設為客供後，該項仍顯示在明細中，但金額不會計入總價。", ["export", "local-storage"]);
  ...
  defineHelp("mobile-bottom-toolbar", "手機主工具列", "手機操作", "手機版固定的主要入口：牆面、新增、編輯、估價、視圖與更多。", "需要快速進入目前最常用的工具時使用。", ["點選需要的入口。", "設定會在 Bottom Sheet、估價畫面或橫屏側面板開啟。"], ["完整功能仍保留在更多，不會因手機版而刪除。"], "點估價可查看目前配置的模擬金額。", ["mobile-more-menu", "bottom-sheet", "estimate"]);
