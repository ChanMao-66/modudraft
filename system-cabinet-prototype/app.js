...
function selectItem(itemId, objectType = "cabinet", compartmentId = null, rerender = true) {
  ...
  if (rerender) { updateForms(); renderAll(); }
}
...
function addCabinet() { const wall = activeWall(), cabinet = createCabinet(wall.items.filter(i => i.type === "cabinet").length); wall.items.push(cabinet); selectItem(cabinet.id); }
...
function switchView(view) { state.view = view; document.querySelectorAll("[data-view]").forEach(b => b.classList.toggle("active", b.dataset.view === view)); const titles = { floor: ["01", "平面圖 · 牆面配置"], elevation: ["02", "立面圖 · 整面牆配置"], configuration: ["03", "立面配置 · 單櫃內部"], three: ["04", "3D · 整面牆預覽"] }; [el.viewIndex.textContent, el.viewTitle.textContent] = titles[view]; el.threeStage.classList.toggle("hidden", view !== "three"); el.draftCanvas.classList.toggle("hidden", view === "three"); renderAll(); }
function switchPropertyPanel(panel) { state.panel = panel; document.querySelectorAll("[data-panel]").forEach(b => b.classList.toggle("active", b.dataset.panel === panel)); document.querySelectorAll("[data-property-view]").forEach(v => v.classList.toggle("active", v.dataset.propertyView === panel)); }
...
function render3D(){if(!window.THREE)return;dispose3D();const rect=el.threeStage.getBoundingClientRect(),renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(2,devicePixelRatio||1));renderer.setSize(Math.max(1,rect.width),Math.max(1,rect.height));renderer.setClearColor(0xf1f0eb);renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.08;if("outputEncoding" in renderer)renderer.outputEncoding=THREE.sRGBEncoding;el.threeStage.appendChild(renderer.domElement);const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(34,rect.width/rect.height,1,50000);scene.fog=new THREE.Fog(0xf1f0eb,8500,18000);scene.add(new THREE.HemisphereLight(0xfffdf7,0x8a9697,1.18));const key=new THREE.DirectionalLight(0xfff4df,1.45);key.position.set(-2200,4300,3400);key.castShadow=true;key.shadow.mapSize.set(2048,2048);key.shadow.camera.left=-6500;key.shadow.camera.right=6500;key.shadow.camera.top=5500;key.shadow.camera.bottom=-1800;key.shadow.bias=-.00025;scene.add(key);const fill=new THREE.DirectionalLight(0xdce8eb,.48);fill.position.set(3600,2100,1800);scene.add(fill);const wall=activeWall(),layout=calculateLayout(wall),group=new THREE.Group();scene.add(group);const carcass=mat(0xe7e2d8,.78),edge=mat(0x9c958a,.82),door=mat(0xd9d8d2,.66),filler=mat(0xb79a76,.76),metal=mat(0x899296,.24,.72);
  ...
  const used=Math.max(wall.width,projectValidation(wall).used),height=wall.height,roomDepth=Math.max(1500,...wall.items.map(i=>i.type==="filler"?i.depth:cabinetTotalDepth(i)))+500;const floor=box(group,used+700,24,roomDepth,used/2,-12,roomDepth/2-180,mat(0xc9b89f,.88),{type:"room"});floor.castShadow=false;const backWall=box(group,used+700,height+500,24,used/2,(height+500)/2,-22,mat(0xe9e7e1,.94),{type:"room"});backWall.castShadow=false;const sideWall=box(group,24,height+500,roomDepth*.72,-338,(height+500)/2,roomDepth*.36-25,mat(0xe2e5e3,.94),{type:"room"});sideWall.castShadow=false;camera.position.set(used*.68,height*.58,Math.max(2850,used*.9));camera.lookAt(used/2,height*.43,260);state.three={renderer,scene,camera,group,raf:0,orbit:{yaw:0,pitch:0,targetX:used/2,targetY:height*.43,distance:camera.position.z}};bindThreeControls(renderer.domElement);const loop=()=>{state.three.raf=requestAnimationFrame(loop);renderer.render(scene,camera);};loop();}
...
function bindEvents(){
  ...
  el.draftCanvas.onclick=e=>{const rect=el.draftCanvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top,region=[...state.hitRegions].reverse().find(r=>x>=r.x&&x<=r.x+r.w&&y>=r.y&&y<=r.y+r.h);if(region){selectItem(region.itemId,region.type,region.compartmentId,false);if(region.compartmentId){state.panel=region.type==="door"?"doors":region.type==="rod"||region.type==="drawers"?"accessories":"shelves";switchPropertyPanel(state.panel);}updateForms();renderAll();}};
