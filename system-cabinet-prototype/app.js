...
function renderSidebar() {
  ...
  el.duplicateCabinetBtn.disabled = !selectedItem(); el.selectedReadout.textContent = selectedCompartment() ? `${selectedItem().name} · 分格 ${selectedCabinet().compartments.findIndex(c => c.id === selectedCompartment().id) + 1}` : selectedItem()?.name || wall.name;
...
function mat(color,rough=.65,metal=.02){return new THREE.MeshStandardMaterial({color,roughness:rough,metalness:metal});}
function box(group,w,h,d,x,y,z,material,userData={}){const mesh=new THREE.Mesh(new THREE.BoxGeometry(Math.max(.5,w),Math.max(.5,h),Math.max(.5,d)),material);mesh.position.set(x,y,z);mesh.userData=userData;group.add(mesh);return mesh;}
function render3D(){if(!window.THREE)return;dispose3D();const rect=el.threeStage.getBoundingClientRect(),renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(2,devicePixelRatio||1));renderer.setSize(Math.max(1,rect.width),Math.max(1,rect.height));renderer.setClearColor(0xece8df);renderer.shadowMap.enabled=true;el.threeStage.appendChild(renderer.domElement);const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(35,rect.width/rect.height,1,50000);scene.add(new THREE.HemisphereLight(0xffffff,0x6e7378,1.4));const key=new THREE.DirectionalLight(0xffffff,1.2);key.position.set(-2500,4000,3500);scene.add(key);const wall=activeWall(),layout=calculateLayout(wall),group=new THREE.Group();scene.add(group);const carcass=mat(0xe1ddd3),edge=mat(0x8f8980),door=mat(0xd1d3cf),filler=mat(0xc7a35a),metal=mat(0x8c9498,.25,.75);
  ...
  const used=Math.max(wall.width,projectValidation(wall).used),height=wall.height;box(group,used+300,20,1200,used/2,-10,400,mat(0xd8d2c8));camera.position.set(used*.55,height*.62,Math.max(2600,used*.95));camera.lookAt(used/2,height*.45,250);state.three={renderer,scene,camera,group,raf:0,orbit:{yaw:0,pitch:0,targetX:used/2,targetY:height*.45,distance:camera.position.z}};bindThreeControls(renderer.domElement);const loop=()=>{state.three.raf=requestAnimationFrame(loop);renderer.render(scene,camera);};loop();}
