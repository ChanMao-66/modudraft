...
function resizeCanvas() { const rect = el.drawingStage.getBoundingClientRect(), dpr = window.MODUDRAFTSystemMobileController?.getRenderDpr?.() || Math.min(2, devicePixelRatio || 1), cssWidth=Math.max(1,rect.width),cssHeight=Math.max(1,rect.height); el.draftCanvas.width = Math.max(1, Math.round(cssWidth * dpr)); el.draftCanvas.height = Math.max(1, Math.round(cssHeight * dpr)); el.draftCanvas.style.width = `${cssWidth}px`; el.draftCanvas.style.height = `${cssHeight}px`; const ctx = el.draftCanvas.getContext("2d"); ctx.setTransform(dpr,0,0,dpr,0,0); return { ctx, width: cssWidth, height: cssHeight }; }
function render2D() {
...
function bindThreeControls(canvas){let down=false,lastX=0,lastY=0,startX=0,startY=0,mode="orbit",pinchDistance=0,pinchCenter=null;const pointers=new Map(),gestures=window.MODUDRAFTMobileCore?.createGestureManager?.();const touchMetrics=()=>{const points=[...pointers.values()].slice(0,2);if(points.length<2)return{distance:0,center:points[0]||null};return{distance:Math.hypot(points[1].x-points[0].x,points[1].y-points[0].y),center:{x:(points[0].x+points[1].x)/2,y:(points[0].y+points[1].y)/2}}};canvas.style.touchAction="none";canvas.onpointerdown=e=>{down=true;lastX=startX=e.clientX;lastY=startY=e.clientY;if(e.pointerType==="touch"){pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});gestures?.pointerDown(e);if(pointers.size>=2){const current=touchMetrics();mode="pinch";pinchDistance=current.distance;pinchCenter=current.center;}else mode="orbit";}else mode=e.button===1||e.shiftKey?"pan":"orbit";canvas.setPointerCapture(e.pointerId);};canvas.onpointerup=e=>{const result=e.pointerType==="touch"?gestures?.pointerUp(e):null;pointers.delete(e.pointerId);down=pointers.size>0;if(result?.suppressTap||Math.hypot(e.clientX-startX,e.clientY-startY)>5){pinchDistance=0;pinchCenter=null;return;}const rect=canvas.getBoundingClientRect(),pointer=new THREE.Vector2((e.clientX-rect.left)/Math.max(1,rect.width)*2-1,-(e.clientY-rect.top)/Math.max(1,rect.height)*2+1),ray=new THREE.Raycaster();ray.setFromCamera(pointer,state.three.camera);const target=ray.intersectObjects(state.three.group.children,true).find(hit=>hit.object.userData?.itemId);if(target){const data=target.object.userData;selectItem(data.itemId,data.type,data.compartmentId||null);}};canvas.onpointermove=e=>{if(!down||!state.three)return;if(e.pointerType==="touch"){if(!pointers.has(e.pointerId))return;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});gestures?.pointerMove(e);if(pointers.size>=2){const current=touchMetrics(),target=new THREE.Vector3(state.three.orbit.targetX,state.three.orbit.targetY,220);if(pinchCenter){state.three.camera.position.x-=(current.center.x-pinchCenter.x)*3;state.three.camera.position.y+=(current.center.y-pinchCenter.y)*3;}if(pinchDistance>0&&current.distance>0){const offset=state.three.camera.position.clone().sub(target).multiplyScalar(pinchDistance/current.distance);state.three.camera.position.copy(target.clone().add(offset));}state.three.camera.lookAt(target);pinchDistance=current.distance;pinchCenter=current.center;mode="pinch";return;}}const dx=e.clientX-lastX,dy=e.clientY-lastY,last=state.three.camera.position.clone();if(mode==="pan"){state.three.camera.position.x-=dx*3;state.three.camera.position.y+=dy*3;}else{const target=new THREE.Vector3(state.three.orbit.targetX,state.three.orbit.targetY,220),offset=last.sub(target),spherical=new THREE.Spherical().setFromVector3(offset);spherical.theta-=dx*.008;spherical.phi=clamp(spherical.phi+dy*.008,.22,Math.PI/2.05,1);state.three.camera.position.copy(target.clone().add(new THREE.Vector3().setFromSpherical(spherical)));state.three.camera.lookAt(target);}lastX=e.clientX;lastY=e.clientY;};canvas.onpointercancel=e=>{if(e.pointerType==="touch")gestures?.pointerCancel(e);pointers.delete(e.pointerId);down=false;pinchDistance=0;pinchCenter=null;};canvas.onwheel=e=>{e.preventDefault();const target=new THREE.Vector3(state.three.orbit.targetX,state.three.orbit.targetY,220),offset=state.three.camera.position.clone().sub(target).multiplyScalar(e.deltaY>0?1.1:.9);state.three.camera.position.copy(target.clone().add(offset));state.three.camera.lookAt(target);};}
